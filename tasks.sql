...
SELECT AVG(Income) as avg_salary FROM customer_data;
...
SELECT ID, NumWebVisitsMonth AS no_web_visits  FROM customer_data WHERE NumWebVisitsMonth > 3 ORDER BY ID;

# 5. List customers who have accepted at least one campaign:

-- USING WHERE Clause
SELECT ID FROM customer_data 
WHERE AcceptedCmp1 = 1 OR
	  AcceptedCmp2 = 1 OR
      AcceptedCmp3 = 1 OR
      AcceptedCmp4 = 1 OR
      AcceptedCmp5 = 1 
